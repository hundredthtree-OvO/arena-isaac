...
class Semantic2DCollisionGuard:
    ...
    def _collision_score(self, pos_xyz, heading: float) -> CollisionResult:
        ...
        x = float(pos_xyz[0])
        y = float(pos_xyz[1])
        rhx = max(0.01, 0.5 * self.config.length + self.config.margin)
        rhy = max(0.01, 0.5 * self.config.width + self.config.margin)
        best = 0.0
        best_name = None
        best_kind = None
        for obs in self.rects:
            score = _obb_penetration(x, y, heading, rhx, rhy, obs.cx, obs.cy, obs.yaw, 0.5 * obs.sx, 0.5 * obs.sy)
            if score > best:
                best = score
                best_name = obs.name
                best_kind = obs.kind
        for obs in self.circles:
            score = _obb_circle_penetration(x, y, heading, rhx, rhy, obs.cx, obs.cy, obs.radius)
            if score > best:
                best = score
                best_name = obs.name
                best_kind = obs.kind

"""Primary voxel-based runtime guard for mobile-base navigation.
...
V20 goal
--------
Static voxels remain the primary obstacle source, but live pedestrians are
checked here as dynamic circular obstacles so the main runtime path no longer
depends on the legacy KinematicCollisionGuard wrapper for actor blocking.
"""
...
try:
    from isaac_utils.dynamic_actor_guard import (
        RobotGuardState,
        footprint_extents as dynamic_footprint_extents,
        predict_robot_pose,
...
class VoxelGuardConfig:
    ...
    overlap_deadband: float = 0.005
    escape_tolerance: float = 0.002
    last_free_max_age_sec: float = 8.0
    zero_eps: float = 1e-4
...
def config_from_env() -> VoxelGuardConfig:
    return VoxelGuardConfig(
        ...
        overlap_deadband=_env_float("ARENA_ISAAC_COLLISION_GUARD_OVERLAP_DEADBAND", 0.005),
        escape_tolerance=_env_float("ARENA_ISAAC_COLLISION_GUARD_ESCAPE_TOLERANCE", 0.002),
        last_free_max_age_sec=_env_float("ARENA_ISAAC_COLLISION_GUARD_LAST_FREE_MAX_AGE_SEC", 8.0),
...
def _footprint_extents(config: VoxelGuardConfig) -> Tuple[float, float, float, float]:
...
class VoxelCollisionGuard:
    """Primary runtime guard backed by occupied voxel columns + live pedestrians."""
    ...
    def __init__(self, *, robot_name: str, logger=None, config: Optional[VoxelGuardConfig] = None):
        ...
        self._last_free_pose_xy: Optional[Tuple[float, float]] = None
        self._last_free_heading: float = 0.0
        self._last_free_time: float = 0.0
        self.query_count = 0
        self.block_count = 0
    ...
    def _collision_score(self, pos_xyz, heading: float) -> Tuple[float, Optional[str], float]:
        ...
        center_x_local, center_y_local, half_len, half_wid = _footprint_extents(self.config)
        half_len = max(0.01, half_len + pad)
        half_wid = max(0.01, half_wid + pad)
        ch = math.cos(heading)
        sh = math.sin(heading)
        center_x = x + ch * center_x_local - sh * center_y_local
        center_y = y + sh * center_x_local + ch * center_y_local
        ox, oy, _ = self.origin
        best = 0.0
        best_key: Optional[Tuple[int, int]] = None
        for ix, iy in self._candidate_indices(center_x, center_y, heading, half_len, half_wid):
    ...
    def _robot_guard_state(self, *, pos_xyz, heading: float, vx: float, vy: float, wz: float, dt: float) -> RobotGuardState:
        forward, rear, left, right = dynamic_footprint_extents(
        ...
        *,
        ...
        for ped in self._dynamic_pedestrian_states(dt):
            ...
            if score <= max(best_current, best_next):
                continue
            best_current = float(current_score)
            best_next = float(next_score)
            best_name = f"pedestrian:{ped.name}"
    ...
    def _effective_score(self, score: float) -> float:
        if score <= self.config.overlap_deadband:
            return 0.0
        return float(score)

    def _update_last_free(self, pos_xyz, heading: float) -> None:
        self._last_free_pose_xy = (float(pos_xyz[0]), float(pos_xyz[1]))
        self._last_free_heading = float(heading)
        self._last_free_time = time.monotonic()

    def _last_free_candidate(self, *, pos_xyz, heading: float, speed_hint: float) -> Optional[Tuple[float, float, float, str]]:
        if self._last_free_pose_xy is None:
            return None
        age = time.monotonic() - self._last_free_time
        if age > self.config.last_free_max_age_sec:
            return None
        dx_w = float(self._last_free_pose_xy[0] - float(pos_xyz[0]))
        dy_w = float(self._last_free_pose_xy[1] - float(pos_xyz[1]))
        dist = math.hypot(dx_w, dy_w)
        if dist < self.config.zero_eps:
            return None
        c = math.cos(heading)
        s = math.sin(heading)
        vx_b = c * dx_w + s * dy_w
        vy_b = -s * dx_w + c * dy_w
        scale = max(float(speed_hint), 0.12) / max(dist, 1e-6)
        return float(vx_b * scale), float(vy_b * scale), 0.0, "return_last_free"
    ...
    def _candidate_is_allowed_from_overlap(self, *, pos_xyz, heading: float, vx: float, vy: float, wz: float, dt: float):
        ...
        current_eff = self._effective_score(current_score)
        if current_eff <= 0.0:
            self._update_last_free(pos_xyz, heading)
            ...
            if dyn_next_score > next_score:
                ...
                next_path = dyn_path
            next_eff = self._effective_score(next_score)
            if next_eff <= 0.0:
        ...
        if next_eff <= 0.0:
            self._maybe_log_inside(path, current_score=current_score, next_score=next_score, action="escape_to_free")
            return True, "inside_escape_to_free", current_score, next_score, path
        if next_eff <= current_eff + self.config.escape_tolerance:
            self._maybe_log_inside(path, current_score=current_score, next_score=next_score, action="escape_tolerated")
            return True, "inside_escape_tolerated", current_score, next_score, path
        if next_eff + self.config.escape_epsilon < current_eff:
    ...
    def filter_motion(self, *, pos_xyz, heading: float, vx: float, vy: float, wz: float, dt: float):
        ...
        last_free = self._last_free_candidate(
            pos_xyz=pos_xyz,
            heading=heading,
            speed_hint=max(abs(vx), abs(vy), 0.12),
        )
        if last_free is not None:
            candidates.insert(0, last_free)

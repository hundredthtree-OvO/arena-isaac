...
class MecanumRobot:
    ...
    def _ensure_initialized(self):
        ...
        try:
            ...
            self._log_info(
                ...
                f"arm_hold={self.config.arm_hold_positions}, "
    ...
    def _apply_arm_hold_targets(self):
        if self._articulation is None or not self._arm_joint_indices:
            return
        targets = np.array(self.config.arm_hold_positions[: len(self._arm_joint_indices)], dtype=np.float32)
        indices = np.array(self._arm_joint_indices, dtype=np.int32)
        ...
        try:
            ...
            pass
        if target_applied and not self.config.arm_hold_hard_sync:
            return
        try:
        ...
        try:
            ...
            self._log_warn(f"[{self.name}] failed to apply arm hold targets: {exc}")

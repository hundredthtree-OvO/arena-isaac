...
from isaac_utils.dual_lidar import create_dual_lidar
try:
...
def urdf_to_usd(request, response):
    ...
    if is_mecanum_model(robot_model) and mecanum_mode == "physx_wheels" and not prebuilt_usd_path:
        raise FileNotFoundError("Prebuilt PhysX mecanum asset not found for mode=physx_wheels")
    if prebuilt_usd_path:
        if not _reference_usd_into_stage(prebuilt_usd_path, prim_path):
            raise RuntimeError(f"Failed to reference prebuilt mecanum asset {prebuilt_usd_path} at {prim_path}")
        usd_path = prebuilt_usd_path
    ...
    if dual_lidar_mecanum:
        try:
            create_dual_lidar(
                robot_root_path=controller_prim_path,
                robot_name=name,
                logger=None,
            )
        except Exception as exc:
    ...
    if stable_kinematic_mecanum:
        # The imported robot is deliberately made non-physical.  Do not create
        # ROS bridge odom/tf/joint_state graphs that target base_link as a
        # PhysX articulation/rigid body, otherwise Isaac 4.5 spams tensor errors
        # such as "Pattern ... did not match any rigid bodies".
        print(
            f"[urdf_import] Skipping Isaac ROS bridge odom/tf/joint_states for stable kinematic mecanum {name}",
            file=sys.stderr,
        )
    else:
    ...
    if stable_kinematic_mecanum or prebuilt_physx_mecanum:
        reason = "stable kinematic mecanum" if stable_kinematic_mecanum else "prebuilt physx mecanum"

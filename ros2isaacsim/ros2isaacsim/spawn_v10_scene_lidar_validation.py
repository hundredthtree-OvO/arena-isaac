#!/usr/bin/env python3
"""Spawn a static validation scene + mecanum robot + dual lidar.
...
This is the Milestone-C/V10 entry point used before pedestrian tests:
  ...
  2. Spawn the custom mecanum robot in the requested mecanum mode.
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info("V10 validation ready. For synthetic 2D lidar, run: arena_scene_profile.py ... synthetic_laser")
    finally:
        node.destroy_node()
        rclpy.shutdown()
